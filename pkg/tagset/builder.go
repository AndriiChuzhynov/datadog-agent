...
//
// An builder goes through three stages in its lifecycle:
//
// * adding tags (begins on call to foundry.NewBuilder).
// * frozen (begins on call to bldr.Freeze).
// * closed (begins on call to bldr.Close).
//
...
// before use.
func newBuilder(foundry Foundry) *Builder {
	return &Builder{
		foundry: foundry,
		tags:    nil,
		frozen:  nil,
...
func (bldr *Builder) Freeze() *Tags {
	if bldr.frozen == nil {
		bldr.frozen = bldr.foundry.getCachedTags(byTagsetHashCache, bldr.hash, func() *Tags {
...
func (bldr *Builder) Close() {
	bldr.seen = nil // free unnecessary memory
	bldr.foundry.builderClosed(bldr)
}
